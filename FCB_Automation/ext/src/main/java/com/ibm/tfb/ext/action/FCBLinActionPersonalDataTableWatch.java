...
public class FCBLinActionPersonalDataTableWatch extends DPFTActionTableWatch {
	...
	*/
	...
	public void postAction() throws DPFTRuntimeException {
		...
		/* Get Data set from "D_LIN" */
		DPFTDboSet dLinSet = ((DPFTActionTableWatch) this.getPreviousAction()).getDataSet();

		/* Set Query criteria for "O_LIN" */
		String qString = DPFTUtil.getFKQueryString(dLinSet.getDbo(0));
		if (qString == null) {
			DPFTLogger.debug(this, "Built FK Query String Failed...");
			return;
		}

		/* Get Data set from "O_LIN" */
		DPFTConnector connector = DPFTConnectionFactory.initDPFTConnector(config);
		DPFTOutboundDboSet oLinSet = (DPFTOutboundDboSet) connector.getDboSet("O_LIN", qString);
		oLinSet.load();
		...
		sb.append(TFBUtil.getCustomerSelectINString(((DPFTActionTableWatch) this.getPreviousAction()).getDataSet(),
				"customer_id"));
		sb.append(")");
		DPFTDboSet dCustIDSet = (DPFTDboSet) connector.getDboSet(" CMDM.ID_DEC(customer_id) AS D_CUST_ID, customer_id as CUST_ID ", "D_LIN", sb.toString());
		...
		if (dLinSet.getDbo(0).getString("MGM_FLAG").equals("Y")) {
		...
		for (int i = 0; i < dLinSet.count(); i++) {
			if ( i == 0 | i%10000 == 0 | i == dLinSet.count()-1 ) {
				DPFTLogger.info(this, "Successfully processed " + String.valueOf(i) + " IDs in LIN channel.");
			}
			DPFTDbo dLin = dLinSet.getDbo(i);
			String cust_id = dLin.getString("customer_id");

			DPFTOutboundDbo new_dbo = (DPFTOutboundDbo) oLinSet.add();
			new_dbo.setValue(dLinSet.getDbo(i));
			
			// Check if appointment time has expired
			String appointmentTime = dLin.getString("START_TIME");
			...
			if ( is_expire ) {
				...
				//處理MGM_ID
				String _DID = "";
				if ( dLin.getString("MGM_FLAG").equals("Y")) {
					try {
				...
				if ( dLin.getString("MGM_FLAG").equals("Y") && (_DID == null || ("").equals(_DID)) ) {
		...
		DPFTLogger.info(this, "Processed total " + dLinSet.count() + ", process time = "
				+ (ps_fin_time - ps_start_time) / 60000 + " min.");
		oLinSet.setRefresh(false);
		oLinSet.save();

		/* Write usage codes to O_USAGECODE Table */
		TFBUtil.processUsageCode(oLinSet, "LIN");

		/* Write results to H_OUTBOUND Table */
		TFBUtil.generateObndCtrlRecord(connector, oLinSet, cell_code_list, cell_name_list, "LIN", true);
		dLinSet.close();
		oLinSet.close();
		dCustIDSet.close();
		dLinSet = null;
