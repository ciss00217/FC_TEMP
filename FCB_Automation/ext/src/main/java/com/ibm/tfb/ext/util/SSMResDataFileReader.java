	...
	public void write2TargetTable(String timestamp) throws DPFTRuntimeException {
		...
		for (HashMap<String, String> rowdata : read_data) {
			...
			for (String col : rowdata.keySet()) {
				...
				if (col.equals("DESTNAME")) {
					String[] DESTNAME = rowdata.get(col).split("\\$\\#");
					if (DESTNAME != null && DESTNAME.length > 2) {
						new_data.setValue("TREATMENT_CODE", DESTNAME[1]);
						new_data.setValue("CUSTOMER_ID", DESTNAME[2]);
					}
				} else {
