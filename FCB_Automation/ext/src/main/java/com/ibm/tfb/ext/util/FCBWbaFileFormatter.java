...
public class FCBWbaFileFormatter extends DPFTCSVFileFormatter {

	public FCBWbaFileFormatter(DPFTFileMetaData h_meta, DPFTFileMetaData d_meta) {
		super(h_meta, d_meta);
	}
	...
	protected void formatControlFileString(DPFTDboSet rs) throws DPFTRuntimeException {
		...
		sb.append(this.getDataFileName()).append(this.getDelimeter());
		...
		if (getDataFileMeta().isStaticColLength()) {
			...
			if (meta.getActionID() != null)
				sb.append(meta.getActionID()).append(this.getDelimeter()).append(num_output);
			else
