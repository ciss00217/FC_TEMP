...
public class BFMBytesBuffer {
	...
	public BFMBytesBuffer(File read_file, String encode) throws IOException {
		if (read_file.exists()) {
	...
	private void process2LineBytes() {
		...
		DPFTLogger.info(this, "all_bytes.length; "+all_bytes.length);
		for (int i = 0; i < all_bytes.length; i++) {
			if (i + 1 < all_bytes.length) {
				if ((char) all_bytes[i] == '\n') {
					DPFTLogger.info(this, "CRLF reach 2! "+i);
					// CRLF reach
					line_count++;
