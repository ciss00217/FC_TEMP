...
public class FCBRmbActionPersonalDataTableWatch extends DPFTActionTableWatch {
	...
	*/
	...
	public void postAction() throws DPFTRuntimeException {
		...
		DPFTDboSet dRmbSet = ((DPFTActionTableWatch) this.getPreviousAction()).getDataSet();
		...
		/* Set Query criteria for "O_RMB" */
		String qString = DPFTUtil.getFKQueryString(dRmbSet.getDbo(0));
		if (qString == null) {
			DPFTLogger.debug(this, "Built FK Query String Failed...");
			return;
		}

		/* Get Data set from "O_RMB" */
		DPFTConnector connector = DPFTConnectionFactory.initDPFTConnector(config);
		DPFTOutboundDboSet oRmbSet = (DPFTOutboundDboSet) connector.getDboSet("O_RMB", qString);
		oRmbSet.load();
		...
		sb.append(TFBUtil.getCustomerSelectINString(((DPFTActionTableWatch) this.getPreviousAction()).getDataSet(),
				"customer_id"));
		sb.append(")");
		DPFTDboSet dCustIDSet = (DPFTDboSet) connector.getDboSet(" CMDM.ID_DEC(customer_id) AS D_CUST_ID, customer_id as CUST_ID ", "D_RMB", sb.toString());
		...
		if (dRmbSet.getDbo(0).getString("MGM_FLAG").equals("Y")) {
		...
		for (int i = 0; i < dRmbSet.count(); i++) {
			if ( i == 0 | i%10000 == 0 | i == dRmbSet.count()-1 ) {
				DPFTLogger.info(this, "Successfully processed " + String.valueOf(i) + " IDs in RMB channel.");
			}
			DPFTDbo dRMB = dRmbSet.getDbo(i);
			String cust_id = dRMB.getString("customer_id");

			DPFTOutboundDbo new_dbo = (DPFTOutboundDbo) oRmbSet.add();
			new_dbo.setValue(dRmbSet.getDbo(i));
			...
			//處理MGM_ID
			String _DID = "";
			if ( dRMB.getString("MGM_FLAG").equals("Y")) {
				try {
			...
			if ( dRMB.getString("MGM_FLAG").equals("Y") && (_DID == null || ("").equals(_DID)) ) {
		...
		DPFTLogger.info(this, "Processed total " + dRmbSet.count() + ", process time = "
				+ (ps_fin_time - ps_start_time) / 60000 + " min.");
		oRmbSet.setRefresh(false);
		oRmbSet.save();

		/* Write usage codes to O_USAGECODE Table */
		TFBUtil.processUsageCode(oRmbSet, "RMB");

		/* Write results to H_OUTBOUND Table */
		TFBUtil.generateObndCtrlRecord(connector, oRmbSet, cell_code_list, cell_name_list, "RMB", true);
		dRmbSet.close();
		oRmbSet.close();
		dCustIDSet.close();
		dRmbSet = null;
