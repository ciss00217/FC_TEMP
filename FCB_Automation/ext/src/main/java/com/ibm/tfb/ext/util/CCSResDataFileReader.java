...
public class CCSResDataFileReader extends DPFTFileReader {

	public CCSResDataFileReader(String dir, ResFileDataLayoutDbo resFileDataLayoutDbo, String chal_name) {
	...
	public void write2TargetTable(String timestamp) throws DPFTRuntimeException {
		...
		targetSet.reset("data_date='" + getDD() + "'");
		...
		for(HashMap<String, String> rowdata: read_data){
			...
			new_data.setValue("data_date", getDD());
		}
		targetSet.save();
	}

	private String getDD() {
		String[] fs = fname.split(GlobalConstants.FILE_DELIMETER_DOT);
		return fs[1];
	}
}
