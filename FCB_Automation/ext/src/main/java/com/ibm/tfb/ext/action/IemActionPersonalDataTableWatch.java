...
public class IemActionPersonalDataTableWatch extends DPFTActionTableWatch {

	@Override
	public DPFTConfig getDBConfig() {
		/* set Table watch db connection properties */
		return TFBUtil.getMKTDMConfig("MKTDMHNIT");
	}

	@Override
	public String getTableName() {

		return DPFTEngine.getSystemProperties("mktdb.tbl.cust.cont.HNIT");
	}
	...
	public void postAction() throws DPFTRuntimeException {
		...
		DPFTOutboundDboSet oEdmSet = (DPFTOutboundDboSet) connector.getDboSet("O_IEM", qString);
		oEdmSet.load();
		...
		for (int i = 0; i < dEdmSet.count(); i++) {
			...
			String email = custSet.getContactInfoByCont("EMAIL", cust_id);
			...
			// email =
			// custSet.getPrioritizedEmail(dEdm.getString("customer_id"), "IEM",
			// GlobalConstants.DPFT_DEFAULT_PRIORITY_CODE);
			// }else{
			// /*use email_priority Setting*/
			// email =
			// custSet.getPrioritizedEmail(dEdm.getString("customer_id"), "IEM",
			// dEdm.getString("email_priority"));
			// }
//			DPFTLogger.info(this, "Get email:" + email);
			if (email == null) {
				...
				// DPFTLogger.info(this, "Set email:" + email);
		...
		TFBUtil.processUsageCode(oEdmSet, "IEM");

		/* Write results to H_OUTBOUND Table */
		TFBUtil.generateObndCtrlRecord(connector, oEdmSet, cell_code_list, cell_name_list, "IEM", true);
		oEdmSet.close();
		dEdmSet.close();
