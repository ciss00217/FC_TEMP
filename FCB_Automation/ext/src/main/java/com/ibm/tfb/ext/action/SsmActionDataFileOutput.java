...
import com.ibm.tfb.ext.util.SsmFileFormatter;

public class SsmActionDataFileOutput extends DPFTActionDataFileOutput {
	@Override
	public DPFTFileFormatter getFileFormatter() throws DPFTRuntimeException {
		String[] header = { "DLV_TIME" };
		return new SsmFileFormatter(new DPFTFileMetaData(meta), new DPFTFileMetaData(meta, dicSet),
				TFBUtil.buildHeaderString(header, this.getPreviousAction().getResultSet().getDbo(0), ""));
	}
